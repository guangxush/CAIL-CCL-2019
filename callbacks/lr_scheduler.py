...
class LRScheduler(Callback):
    ...
    def plot_loss(self, valid_loss=True):
        """
    ...
    def plot_acc(self, valid_acc=True):
        """
...
class LRRangeTest(LRScheduler):
    ...
    def on_batch_end(self, epoch, logs=None):
        ...
        loss = logs['loss']
        if (math.isnan(loss) or loss > self.best_loss * 10) and K.get_value(self.model.optimizer.lr) > 1e-2:
...
class CyclicLR(LRScheduler):
    ...
    Class also supports custom scaling functions:
        ```python
            clr_fn = lambda a_segment: 0.5*(1+np.sin(a_segment*np.pi/2.))
            clr = CyclicLR(base_lr=0.001, max_lr=0.006,
    ...
    # Arguments
        ...
        step_size: number of training iterations per
            ...
            2-8 a_segment training iterations in epoch.
        ...
        scale_fn: Custom scaling policy defined by a single
            ...
            0 <= scale_fn(a_segment) <= 1 for all a_segment >= 0.
    ...
    def update_lr(self):
        ...
        if self.scale_mode == 'cycle':
            ...
            lr = self.base_lr + (self.max_lr - self.base_lr) * np.maximum(0, (1 - x)) * self.scale_fn(
                self.clr_iterations)
        K.set_value(self.model.optimizer.lr, lr)
